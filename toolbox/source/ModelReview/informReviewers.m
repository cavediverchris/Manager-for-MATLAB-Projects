...
%   The purpose of this utility function is to poll the status of all
%   models in the project for their review state. If the review state (for
%   either a Peer Review, or Programme Review) is set to "To Review" then
%   the user will be given a warning alerting them to this fact.   
%
...
%   FUNCTION OUTPUTS
%       OUTPUT 1: result
%           Dimensions and type: 1x1 boolean
%           Description: Reminders in production code is not acceptable.
%           Therefore this function represents a test and the output result
%           represents the outcome of that test. If result is TRUE then the
%           test has passed and there are no code reminders. If the result
%           is FALSE then the test was failed and code reminders are
%           present. The Command Window will contain an output of the
%           file(s) with a count for each reminder(s).
%
%       OUTPUT 1: resultData
%           Dimensions and type: nx3 table
%           Description: When the test is passed, this output will be empty
%           ([]). When the test has been failed. 
%
%   OTHER OUTPUTS
%       None
%
