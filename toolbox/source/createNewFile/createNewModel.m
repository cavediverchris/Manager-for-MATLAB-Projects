...
if strcmp(FileType, 'Script')
    ...
    ParentFolder = strcat(highLevelScriptsFolder, '\', NewScriptName);
    
    if exist(ParentFolder, 'dir') == 0 
        % CASE: The folder for the new function doesn't exist
        % ACTION: Create it
        mkdir(ParentFolder);
...
% Check if SL Reqts is available
if status == true
    if strcmp(FileType, 'Function')
    ...
    disp('Skipping creating requirements modules.')
end
...
if strcmp(FileType, 'Script')
    movefile(which(NewScriptName), strcat(ParentFolder, '\'));
elseif strcmp(FileType, 'Function')
    movefile(which(NewScriptName), strcat(ParentFolder, '\'));
    movefile(which(th_name), strcat(ParentFolder, '\', th_name));
end
